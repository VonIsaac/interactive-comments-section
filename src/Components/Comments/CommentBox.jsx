...
export default function CommentBox({comments, onDeletes}){
    ...
    const opendDeleteModal = useRef()

    function openModal(){
        opendDeleteModal.current.open()
    }
            ...
            <ul>     
                {comments.map((replycomment) => (
                    <div className='border-0 border-slate-950 w-[73%] p-4 mb-4 bg-slate-100 rounded-xl ml-10'>
                         <ModalDelete ref={opendDeleteModal} button = "NO CANCEL" onDeletes = {() => onDeletes(replycomment.id)}  >
                        ...
                        <li key={replycomment.id}>
                            <div  className='flex gap-4 mb-2'>
                                ...
                                <button onClick={openModal} className=' ml-auto font-bold mb-3 cursor-pointer text-rose-700  hover:text-slate-950'>Delete</button>
                            </div>
